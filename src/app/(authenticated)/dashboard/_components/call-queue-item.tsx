...
import { Hash, Phone, PhoneOutgoing } from "lucide-react";
  ...
  return (
    <div className="flex items-center p-3 text-base font-bold text-gray-900 rounded-lg bg-gray-50 hover:bg-gray-100 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white">
      ...
      <div className="flex flex-1 flex-col">
        <span className="flex ms-3 whitespace-nowrap truncate items-center">
          {name}
          {tag && (
            <span className="inline-flex items-center justify-center px-2 py-0.5 ms-3 text-xs font-medium text-gray-500 bg-gray-200 rounded dark:bg-red-700 dark:text-white">
              {tag}
            </span>
          )}
        </span>
        <span className="flex flex-row gap-2 items-center ms-3 text-sm text-gray-500 dark:text-gray-400">
          <PhoneOutgoing className="w-3 h-3" /> {duration}
        </span>
        <span className="flex flex-row gap-2 items-center ms-3 text-sm text-gray-500 dark:text-gray-400">
      ...
      <div>
        <Button
          ...
          asChild
        >
          <Link href={`/call/${id}`} className="gap-2">
